/**
 * Test real Google OAuth JWT with oidc_domain_attestation circuit.
 *
 * Usage: npx tsx scripts/test-real-google-jwt.ts
 *
...
// ---------------------------------------------------------------------------
async function main() {
  console.log("=== OIDC Domain Attestation — Real Google JWT Test ===\n");

  // 1. Get JWT from gcloud
  console.log("1. Getting Google identity token via gcloud...");
  const jwt = execSync("gcloud auth print-identity-token", { encoding: "utf-8" }).trim();
  ...
  if (atIndex === -1) throw new Error("No @ in email");
  const domain = email.substring(atIndex + 1);
  console.log(`   Domain extracted: "${domain}"\n`);

  // 3. Fetch Google JWKS
  console.log("3. Fetching Google JWKS...");
  ...
  if (!matchingKey) {
    throw new Error(`No JWKS key matching kid="${header.kid}". Available kids: ${jwks.keys.map((k: any) => k.kid).join(", ")}`);
  }
  console.log(`   Found matching key: kid=${header.kid}\n`);

  // 4. Compute RSA limbs
  console.log("4. Computing RSA limbs...");
  ...
  const modulusBuf = base64urlToBuffer(matchingKey.n as string);
  ...
  console.log("   18 × 120-bit limbs computed.\n");

  // 5. Partial SHA-256
  console.log("5. Computing partial SHA-256...");
  const payloadJson = Buffer.from(payloadB64, "base64url").toString("utf-8");
  const emailKeyIndex = payloadJson.indexOf('"email"');
  if (emailKeyIndex === -1) throw new Error("Could not find 'email' key in payload");
  ...
  console.log(`   SHA cutoff index: ${signedData.length - remainingData.length}`);
  console.log(`   Remaining data length: ${remainingData.length}`);

  if (remainingData.length > MAX_PARTIAL_DATA_LENGTH) {
    throw new Error(`remainingData (${remainingData.length}) exceeds MAX_PARTIAL_DATA_LENGTH (${MAX_PARTIAL_DATA_LENGTH})`);
  }
  ...
  console.log(`   partialHash: [${Array.from(partialHash).map(v => (v>>>0).toString()).join(", ")}]\n`);

  // 6. Domain
  console.log("6. Computing domain BoundedVec...");
  ...
  // 7. Scope
  console.log("7. Computing scope...");
  const scopeBytes = keccak256Bytes(new TextEncoder().encode(SCOPE_STRING));
  console.log(`   scope = keccak256("${SCOPE_STRING}")\n`);

  // 8. Nullifier
  console.log("8. Computing nullifier...");
  ...
  console.log(`   nullifier = keccak256(keccak256("${email}") ++ scope)\n`);

  // 9. Write Prover.toml
  console.log("9. Writing Prover.toml...");
  const proverToml = `# ===================================================================
# OIDC Domain Attestation — Real Google JWT Inputs
...
`;
  ...
  console.log("=== Prover.toml generated. Run build script to test proof generation. ===");
}
